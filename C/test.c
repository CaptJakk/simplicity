...
static void test_elements(void) {
  printf("Test elements\n");
  {
    ...
    transaction* tx1 = mallocTransaction(&testTx1);
    if (tx1) {
      successes++;
      bool execResult;
      {
        FILE* file = fmemopen_rb(elementsCheckSigHashAllTx1, sizeof_elementsCheckSigHashAllTx1);
        if (execSimplicity(&execResult, tx1, 0, elementsCheckSigHashAllTx1_cmr, elementsCheckSigHashAllTx1_wmr, file) && execResult) {
          successes++;
        } else {
          failures++;
      ...
      }
      {
        ...
        if (execSimplicity(&execResult, tx1, 0, NULL, NULL, file) && !execResult) {
          successes++;
        } else {
          failures++;
  ...
  /* test a modified transaction with the same signature. */
  {
    ...
    transaction* tx2 = mallocTransaction(&testTx2);
    if (tx2) {
      successes++;
      bool execResult;
      {
        FILE* file = fmemopen_rb(elementsCheckSigHashAllTx1, sizeof_elementsCheckSigHashAllTx1);
        if (execSimplicity(&execResult, tx2, 0, NULL, NULL, file) && !execResult) {
          successes++;
        } else {
          failures++;
